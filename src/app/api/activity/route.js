import { getActivities, insertActivities } from "@/app/lib/database/databaseAPI";

//http://localhost:3000/api/activity?userId=1
...
export async function POST(request) {
    try {
        ...
        const { userId, date, name } = requestData;

        const newActivity = await insertActivities(userId, date, name);
        if (newActivity) {
