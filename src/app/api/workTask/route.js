import {getTasksWork, insertTaskWork} from "@/app/lib/database/databaseAPI";

//http://localhost:3000/api/workTask?userId=1
export async function GET(request) {
    try {
        ...
        const activities = await getTasksWork(userId);
        if (activities) {
...
//http://localhost:3000/api/workTask
// {
//     "userId":1,
//     "date":"19.01.2024",
...
export async function POST(request) {
    try {
        ...
        const newActivity = await insertTaskWork(userId, date, time, name);
        if (newActivity) {
